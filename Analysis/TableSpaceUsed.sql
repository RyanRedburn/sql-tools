...
allocation([object_id], total_space_in_mb, used_space_in_mb, unused_space_in_mb)
AS
(
	SELECT t.[object_id],
		CAST(ROUND((SUM(a.total_pages) * 8) / 1024.00, 2) AS DECIMAL(18, 2)),
		CAST(ROUND((SUM(a.used_pages) * 8) / 1024.00, 2) AS DECIMAL(18, 2)),
...
SELECT s.[name] AS [schema_name], t.[name] AS table_name, r.table_type, r.row_count, r.partition_count,
	a.total_space_in_mb, a.used_space_in_mb, a.unused_space_in_mb, t.lock_escalation_desc, t.is_replicated,
	t.is_published, t.is_merge_published, t.is_sync_tran_subscribed, t.is_tracked_by_cdc, t.is_filetable
FROM result AS r
	JOIN allocation AS a ON a.[object_id] = r.[object_id]
